...
class system:
    ...
    def initSystemData(systemData_in):
        global systemData, player
        systemData = systemData_in
...
class data:
    ...
    class itemData:
        ...
        def createItemEntity(catIn, index):
            cat = invTypeKey[catIn]
            if cat == 'potions':
                item = data.itemData.potionItem(**itemDataList[cat][index]).__dict__
                return item
            elif cat == 'weapons':
                item = data.itemData.weaponItem(**itemDataList[cat][index]).__dict__
                return item
            elif cat == 'armor':
                item = data.itemData.armorItem(**itemDataList[cat][index]).__dict__
                return item
            elif cat == 'shields':
                item = data.itemData.shieldItem(**itemDataList[cat][index]).__dict__
                return item
            elif cat == 'spells':
                item = data.itemData.spellItem(**itemDataList[cat][index]).__dict__
                return item
            elif cat == 'keyItems':
                item = data.itemData.keyItem(**itemDataList[cat][index]).__dict__
                return item
            else:
                debug.log('e', f'cannot create item from cat: \'{cat}\'')
        ...
        class potionItem:
            def __init__(self, id, name, type, healing, value, stackable, tier, quantity):
                self.id = id
                self.name = name
                self.type = type
        ...
        class weaponItem:
            def __init__(self, id, name, type, damage, epCost, critChance, value, stackable, tier, quantity):
                self.id = id
                self.name = name
                self.type = type
                self.damage = damage
        ...
        class armorItem:
            def __init__(self, id, name, type, hp, hpMax, disperse, value, stackable, isBroken, tier, quantity):
                self.id = id
                self.name = name
                self.type = type
                self.hp = hp
        ...
        class shieldItem:
            def __init__(self, id, name, type, hp, hpMax, epCost, disperse, value, stackable, isBroken, tier, quantity):
                self.id = id
                self.name = name
                self.type = type
                self.hp = hp
        ...
        class spellItem:
            def __init__(self, id, name, type, spellType, stats, critChance, stackable, mpCost, tier):
                self.id = id
                self.name = name
                self.type = type
    ...
    class enemyData:
        ...
        def createEnemyEntity(index):
            enemy = data.enemyData.enemyEntity(**enemyDataList[index]).__dict__
            return enemy


        class enemyEntity:
            def __init__(self, id, general, stats, loadout, lootTable):
                self.id = id
                generalReal = {
                    'name': general['name'],
                    'creature': general['creature'],
                    'type': general['type'],
                    'description': general['description'],
                }
                self.general = generalReal
                statsReal = {
                    'level': stats['level'],
                    'lvlRange': stats['lvlRange'],
                    'hp': stats['hp'],
                    'hpMax': stats['hpMax'],
                    'baseHp': stats['baseHp'],
                    'str': stats['str'],
                    'spd': stats['spd'],
                    'healFactor': stats['healFactor'],
                }
                self.stats = statsReal
                loadoutReal = {
                    'weapon': None,
                    'armor': None,
                    'shield': None,
                }
                loadoutReal['weapon'] = (data.itemData.createItemEntity(loadout['weapon'][0], loadout['weapon'][1]))
                loadoutReal['armor'] = (data.itemData.createItemEntity(loadout['armor'][0], loadout['armor'][1]))
                loadoutReal['shield'] = (data.itemData.createItemEntity(loadout['shield'][0], loadout['shield'][1]))
                self.loadout = loadoutReal
...
class gameClass:
    ...
    def givePlayerAllItems(itemDataList):
        for cat in itemDataList:
            for i in range(len(itemDataList[cat])):
                catReal = invTypeKeyReverse[cat]
                itemReturn = data.itemData.createItemEntity(catReal, i)
                inventory.addToInventory(itemReturn, 1)
...
class locationClass:
    ...
    def setPlayerLocation(locationID):
        global player
        for locationKey in locationDataList:
            if locationDataList[locationKey]['id'] == locationID:
                player['data']['locationID'] = locationKey
                break
        else:
            player['data']['locationID'] = None
            debug.log('e', f'[locationClass]: location ID: {locationID} not found')
...
class playerClass:
                    ...
                    if targetItem['type'] == 'potion':
                        if userInput == 'u':
                            if player['stats']['hp'] == player['stats']['hpMax']:
                                ...
                                if player['flags']['isInCombat'] == True:
        ...
        class stats:
            ...
            def healPlayer(heal): # heal player amount of health
                ...
                if hp < 0.20 * hpMax:
                    ...
                    message = ('+ {}{}{} HP  ({}{}{}/{}{}{})'.format(clr1, heal, stop, clr2, hp, stop, clr2, hpMax, stop))
                if player['flags']['isInCombat'] == False:
                    print(message)
    ...
    class menu:
        ...
        def locationMenu(player):
            graphics.scene.locationTop(player)
            graphics.scene.locationMenu(player)
...
class battleStats:
    ...
    def calcPlayerSpellAttack(playerIn, spell):
        ...
        if spell['spellType'] == 'damage':
            baseAttack = random.randint(spell['stats'][0], spell['stats'][1])
            # debug.log('d', 'player baseAttack = {}'.format(baseAttack))
            addAttack = stats['int']
            # debug.log('d', 'player addAttack = {}'.format(addAttack))
            if crit == True:
...
class combat:
    ...
    def combatMenu(playerIn, enemy): # main function for combat interaction
        ...
        def access():
            ...
            if isinstance(userInput, int) == True:
                    if userInput == 1:
                        # WEAPON (ATTACK) [1]
                        if playerIn['loadout']['weapon'] != None:
                        ...
                        inventoryChoice = playerClass.menu.spellOnlyMenu(playerIn)
                        ...
                        inventoryChoice = playerClass.menu.playerMenu()
                        if inventoryChoice != 0:
    ...
    def combatLoop(playerIn, enemyIn):
        ...
        inCombat = True
        combatOver = False
        playerIn['flags']['isInCombat'] = inCombat
    ...
    def playerWins(playerIn, enemy):
        ...
        playerName = playerIn['general']['name']
        print('{}{}{} defeated {}{}{}'.format(clr0, playerName, stop, clr1, enemyName, stop))
        ...
        if playerChoice == 1 or playerChoice == 3: # ATTACK / SPELL
            if playerChoice == 1: # ATTACK WEAPON
                ...
                if inventoryChoice['spellType'] == 'damage':
                    ...
                    playerCombatData['playerHeal'] = inventoryChoice[inventoryChoice['stats']] # think after item data struct change this has to be 'stats', if it is send cookie back into the past (4/8/2021 at 9:40 PM at desk home)
...
def main():
    ...
    playerClass.action.loadout.setWeapon(3)
    playerClass.action.loadout.setArmor(2)
    playerClass.action.loadout.setShield(2)

    locationClass.setPlayerLocation(0)
    # playerClass.menu.locationMenu(player)
    clr0 = textColor['grn_b']
    stop = textColor[defaultColor]

    graphics.clear()
    graphics.line(systemData)
    graphics.asciiGraph.printAscii(ascii.logo.logo1())
    graphics.line(systemData)
    print('\n[{}enter{}]: Begin'.format(clr0, stop))
    graphics.click()

    systemOptions['debug']['levelUpShow'] = False
    stats.setLevel(5)
    systemOptions['debug']['levelUpShow'] = True

    while player['stats']['hp'] > 0:
        player['stats']['hp'] = player['stats']['hpMax']
        genEnemy = data.enemyData.createEnemyEntity(1)
        genEnemy = enemyClass.levelSet(genEnemy, random.randint(0, player['stats']['level'] + 2))
        combat.combatLoop(player, genEnemy)
    else:
        graphics.clear()
        graphics.line(systemData)
        print('\n...')
