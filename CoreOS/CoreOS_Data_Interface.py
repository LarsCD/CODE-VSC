...
class entryData:
	...
	def printEntryList():
		...
		for entryDir in systemData['entries']:
			...
			try:
				for file in listdir(os.path.join(thisDir, entryDir)):
					tryFile = os.path.join(os.path.join(thisDir, entryDir), file)
					if isfile(tryFile):
						if 'systemData' in tryFile or 'documents' in tryFile:
							if entryData.checkEntry(entryDir, pathData.returnPath(entryDir, file)) == False:
								# is not in entries
...
class interface: 
	...
	def command(): # placeholder command
		pass
